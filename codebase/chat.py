...
def tool_results_message(events: list[dict[str, Any]]) -> dict[str, str]:
    return {
        ...
        "content": (
            ...
            "Use only these tool results as factual evidence. Answer the user directly, "
            "concisely, state certainty, and cite official IDs/sources (e.g. ANN-xxx) if available."
        ),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Interactive Discord Assistant Agent chat with transcript logging (Track B - Promaxima).")
    ...
    print(f"Discord Assistant Agent chat (K4 Promaxima - Track B). artifact_version={artifact_version.artifact_version}")
